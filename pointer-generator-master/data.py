...
class Vocab(object):
  ...
  def __init__(self, vocab_file, pretrain_vocab_file, max_size):
    ...
    self._glove_vocab = []#A
    self._glove_embd = []#A
    self._embedding_dict = {}#A
    self._emb_dim = 0

    #A
    with open(pretrain_vocab_file, 'r') as vocab_f:
        for line in vocab_f:
    ...
    self._emb_dim = len(self._glove_embd[0])
    ...
    for w in [UNKNOWN_TOKEN, PAD_TOKEN, START_DECODING, STOP_DECODING]:
      ...
      self._id_to_word[self._count] = w
      self._count += 1
      self._glove_embd.append(np.random.randn(self._emb_dim))
