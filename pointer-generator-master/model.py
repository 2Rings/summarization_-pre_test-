...
class SummarizationModel(object):
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      self._glove_init = tf.constant_initializer(self._vocab._glove_embd)# A

      # Add embedding matrix (shared by the encoder and decoder inputs)
      with tf.variable_scope('embedding'):


        embedding = tf.get_variable('embedding', [vsize, hps.emb_dim], dtype = tf.float32, initializer = self._glove_init, trainable=False)
      ...
      with tf.variable_scope('output_projection'):
        ...
        #w = tf.get_variable('w', [hps.hidden_dim, vsize], dtype=tf.float32, initializer=self._glove_init)

        w_t = tf.transpose(w)
        v = tf.get_variable('v', [vsize], dtype=tf.float32, initializer=self.trunc_norm_init)

        #v = tf.get_variable('v', [vsize], dtype=tf.float32, initializer=self._glove_init)
